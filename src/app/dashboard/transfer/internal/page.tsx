...
export default function InternalTransferPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!accountNumber || !amount || !pin || !softToken) {
    ...
    if (session) {
      const { error } = await supabase.from("transactions").insert({
        ...
        reference: `INT-${Math.floor(Math.random() * 1000000)}`
      });
      ...
      <div className="px-4 md:px-10 pb-20 pt-8 w-full animate-in fade-in duration-500 flex justify-center min-h-screen">
        <div className="w-full max-w-[600px] flex flex-col items-center justify-center pt-20">
          ...
          <p className="text-[15px] text-gray-500 text-center max-w-md mb-8 leading-relaxed">
            Your internal transfer of <strong className="text-gray-900">{currentWallet.isCrypto ? "" : currencySymbol}{amount} {currentWallet.isCrypto ? currentWallet.symbol : ""}</strong> to account <strong className="text-gray-900">{accountNumber}</strong> is currently processing.
          </p>
          <button 
    ...
    <div className="px-4 md:px-10 pb-20 pt-8 w-full animate-in fade-in duration-500 flex justify-center">
      <div className="w-full max-w-[850px] flex flex-col gap-8">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-[24px] font-bold text-gray-900 tracking-tight">Internal Transfer</h1>
            <p className="text-[13px] text-gray-500 mt-0.5">Transfer funds to another OCBC account instantly.</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[1.5rem] p-4 md:p-10 shadow-sm border border-gray-100">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden bg-gray-50/50 px-4 py-3.5">
                <Building2 className="w-5 h-5 text-gray-400 mr-3" />
                <input 
                  type="text" 
                  readOnly
                  value="Oversea-Chinese Banking Corporation"
                  className="w-full outline-none text-[15px] font-medium text-gray-900 bg-transparent"
                />
              </div>
            </div>
