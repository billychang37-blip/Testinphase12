...
export default function CryptoTransferPage() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      if (profileData.soft_token) setSoftToken(profileData.soft_token);
      setLoading(false);
    };
    fetchUser();
  }, [router]);
  ...
  // Calculate equivalent crypto amount (assuming 1 USD = 1 USDT for simplicity)
  const equivalentCrypto = amount ? (parseFloat(amount) * 1.0).toFixed(2) : "0.00";

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErrorMsg("");

    if (!amount || !walletAddress || !pin || !softToken) {
    ...
    if (session) {
      const { error } = await supabase.from("transactions").insert({
        ...
        amount: numAmount,
        status: 'pending',
        description: `${currentWallet.name} Transfer to ${walletAddress.substring(0,6)}...${walletAddress.substring(walletAddress.length-4)}`,
        reference: `CRYPTO-${Math.floor(Math.random() * 1000000)}`
      });
      ...
      <div className="px-4 md:px-10 pb-20 pt-8 w-full animate-in fade-in duration-500 flex justify-center min-h-screen">
        <div className="w-full max-w-[600px] flex flex-col items-center justify-center pt-20">
          ...
          <p className="text-[15px] text-gray-500 text-center max-w-md mb-8 leading-relaxed">
            Your crypto transfer of <strong className="text-gray-900">{currentWallet.isCrypto ? "" : currencySymbol}{amount} {currentWallet.isCrypto ? currentWallet.symbol : ""}</strong> to wallet <strong className="text-gray-900">{walletAddress.substring(0,8)}...</strong> is processing on the blockchain.
          </p>
          <button 
      ...
      <div className="w-full max-w-[850px] flex flex-col gap-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-[24px] font-bold text-gray-900 tracking-tight">Crypto Transfer</h1>
              <p className="text-[13px] text-gray-500 mt-0.5">Securely send crypto to external wallets.</p>
            </div>
          </div>
          <button 
            onClick={() => router.push('/dashboard/transactions')}
            className="text-[14px] text-[#E81C24] font-semibold hover:underline"
          >
            History
          </button>
        </div>
          ...
          <div className="relative z-10 mb-6">
            ...
            {!profile?.soft_token && (
              <button 
                ...
                className="text-[12.5px] text-[#E81C24] font-medium hover:underline mt-4 inline-block"
              >
                (Activate Soft Token)
              </button>
            )}
        ...
        <div className="bg-white rounded-[1.5rem] p-4 md:p-10 shadow-sm border border-gray-100">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="relative pt-2">
              ...
              <div className="absolute right-0 top-0 translate-y-8 mt-2 mr-2">
                <button type="button" className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 rounded-lg bg-white shadow-sm hover:bg-gray-50 text-[12.5px] font-medium text-gray-600 transition-colors">
                  Swap <ArrowRightLeft className="w-3.5 h-3.5" />
                </button>
              </div>

              {/* Conversion Display */}
              <div className="mt-6 p-4 rounded-xl border border-gray-100 bg-gray-50/50 flex flex-col gap-1">
                <div className="text-[12px] font-bold text-gray-500 uppercase tracking-wider">Amount In USDT</div>
                <div className="flex items-center text-[15px] font-medium text-gray-900">
                  <Bitcoin className="w-4 h-4 text-gray-400 mr-2" />
                  {currentWallet.isCrypto && currentWallet.symbol === 'USDT' ? amount || '0.00' : equivalentCrypto}
                </div>
              </div>
            </div>
            ...
            {/* Wallet Address */}
            <div>
              <label className="block text-gray-700 font-medium text-[14px] mb-2">Wallet Address <span className="text-red-500">*</span></label>
              <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden focus-within:border-[#E81C24] transition-colors px-4 py-3.5 bg-white">
                <WalletCards className="w-5 h-5 text-gray-400 mr-3" />
                <input 
                  type="text" 
                  required
                  placeholder="0x..."
                  ...
                  className="w-full outline-none text-[15px] font-medium text-gray-900 bg-transparent placeholder:text-gray-400 font-mono"
                />
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full bg-[#E81C24] hover:bg-[#d41920] disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-4 rounded-xl transition-colors mt-6 text-[15px]"
            >
